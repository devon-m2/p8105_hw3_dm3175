...
As expected, the plots illustrate that the average maximum temperature is consistently lower in Janurary than July. The average temperatures have fluctuated yearly from 1981-2010, although not always significantly. The following summary tables 
...
temp_density_p = clean_NOAA_data %>%
  gather(key = observation, value = temp, tmax_celsius:tmin_celsius) %>%
  ggplot(aes(x = temp, fill = observation)) +
  geom_density(alpha = .5) + 
  viridis::scale_fill_viridis(discrete = TRUE) +
  labs(
    title = "Tmax vs. Tmin for all weather stations, 1981-2010",
    x = "Temperature (C)",
    caption = "Data from the NY_NOAA package"
  )

Snowfall_scatter_p = clean_NOAA_data %>% 
